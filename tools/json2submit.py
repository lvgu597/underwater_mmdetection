...
underwater_classes = {1: 'echinus', 2: 'starfish', 3:'holothurian', 4: 'scallop'}
def parse_args():
...
if __name__ == '__main__':
    args = parse_args()
    test_json_raw = json.load(open("./data_process/testA.json", "r"))
    test_json = json.load(open(args.test_json, "r"))
    submit_file_name = "result_"+""+time.strftime("%Y%m%d%H%M%S", time.localtime())+".csv"
    submit_path = '../data/results/'
    ...
    for imgid, annos in imgid2anno.items():
        for anno in annos:
            ...
            class_name = underwater_classes.get(class_id)
            # print(imgid)
