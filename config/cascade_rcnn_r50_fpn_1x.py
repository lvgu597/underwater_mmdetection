...
train_pipeline = [
    ...
    dict(
        ...
        img_scale=(1024,1024), #原来 600 1200
...
test_pipeline = [
    ...
    dict(
        ...
        img_scale=(1024,1024),
        #img_scale=[(4096, 800), (4096, 1000), (4096, 1200)], #原来 (4096, 1000)
        flip=True, #测试时是否翻转图片
        transforms=[
            # dict()
...
data = dict(
    ...
    train=dict(
        ...
        oversample_thr=0,  #进行oversample操作
        dataset = dict(
            type=dataset_type,
            ann_file=data_root + 'train/annotations/instances_train0702.json',
            img_prefix=data_root + 'train/defect_images/',
            pipeline=train_pipeline)),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'train/annotations/instances_val0702.json',
        img_prefix=data_root + 'train/defect_images/',
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type,
        ann_file= '/home/jkx/project/smallq/underwater/underwater_mmdetection/tools/data_process/test0702.json',
        img_prefix=data_root + 'test/TIFFImages',
        pipeline=test_pipeline))
    # test=dict(
    #     type=dataset_type,
    #     ann_file=data_root + 'train/annotations/instances_val0702.json',
