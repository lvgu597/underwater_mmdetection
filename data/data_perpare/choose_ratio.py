...
# 读取数据
ann_json = '../../coco_data/train/annotations/instances_train0702.json'
...
for a in ann['annotations']:
    if a['category_id'] != 0:
        box_w.append(round(a['bbox'][2],2)) #小数点后两位
        box_h.append(round(a['bbox'][3],2))
        wh = round(a['bbox'][2]/a['bbox'][3],0)
        if wh <1 :
            wh = round(a['bbox'][3]/a['bbox'][2],0) #小于1 转为h/w
        box_wh.append(wh)
