...
for index in range(len(name_list)):
    ...
    save_temp_name = 'template_' + name_list[index].strip('.jpg') + '_' +str(index) + '.jpg'
    ...
    for idx in range(len(bboxs)):
        if idx < 3: #最多贴前三个目标
            ...
            # 筛选长条的采点位置
            if w_h > 10 or h_w >10 or (ymax-ymin)*(xmax-xmin)>300000:#TODO: 这里的两个5感觉有问题，300000的应该丢弃
            ...
            if d_name in aug_name: #进行尺度扩充
                ...
                if mutual_infor>0.8:
                    ...
                    x1 = left_top_x
                    y1 = left_top_y
                    x2 = left_top_x+defect_h*scale
                    y2 = left_top_y+defect_w*scale
                    width = max(0, x2 - x1)
                    height = max(0, y2 - y1)
...
json_name='../../coco_data/annotations/Duck_inject_normal.json'
