...
train_pipeline = [
    ...
    dict(
        ...
        img_scale=[(1600,400),(1600,1400)], #调小尺寸以减少gpu负担
...
test_pipeline = [
    ...
    dict(
        ...
        img_scale=[(600,900),(800,1200),(1000,1500),(1200,1800),(1400,2100)], #最大test scale
        flip=True, #测试时是否翻转图片
        transforms=[
...
data = dict(
    ...
    train=dict(
        ...
        oversample_thr=1e-2,  #进行oversample操作
        dataset = dict(
            type=dataset_type,
            ann_file=data_root + 'annotations/Duck_inject_no_u.json',
...
log_level = 'INFO'
work_dir = '../data/work_dirs/cascade_rcnn_x101_fpn_40e_mixup_gc_dcnv2_dnou' #预训练选择模型更匹配的101层还是任务更匹配的
load_from = '../data/pretrained/cascade_rcnn_x101_64x4d_fpn_1x_coco_20200515_075702-43ce6a30.pth' #加载预训练网络参数
resume_from = '../data/work_dirs/cascade_rcnn_x101_fpn_40e_mixup_gc_dcnv2_dnou/latest.pth'
workflow = [('train', 1)]
